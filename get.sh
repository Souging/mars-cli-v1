...
RPC_URL='https://rpc.shyft.to?api_key=xxxxxxxxxxx'
...
for keyFileName in $(ls -1rt /root/workspace/ore/*.json); do
        ...
        rewards=$(/root/mars-cli/target/release/mars --rpc $RPC_URL --keypair $keyFileName rewards)
        if [ $? -ne 1 ]; then
            rs_array=($rewards)
...
printf "当前总产量: %.9f MARS GAS余额: %.9f SOL\n" $totalRewards $totalBalance
